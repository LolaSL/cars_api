...
function News() {
  ...
  const url = `https://newsapi.org/v2/everything?domains=autocar.co.uk&apiKey=640e78c8d6d7466faf6e27fcfe8102ec`;
  ...
  return (
    ...
    >
      ...
      <p className="text-2xl mt-6 mb-6">
        ...
        Latest News and Videos From AUTOCAR Driver Source. Just Be
        Updated
      </p>
      <CustomButton
        btnType="button"
          ...
          data.articles.map((d, index) => {
            return (
              <div key={index} className="article-card group ">
                ...
                <div className="font-lighter mb-2">{d.author}</div>
                <div className="text-sm text-black-200 mb-2">{d.publishedAt}</div>
                <div  className=" font-extrabold">{d.title}</div>
                {/* <div className="font-lighter mb-2">{d.source.name}</div> */}
                <div>
                  {" "}
                  <Link
                    href={`${d.url}`}
